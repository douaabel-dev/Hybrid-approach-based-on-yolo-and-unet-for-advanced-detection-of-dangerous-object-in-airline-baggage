...
def fuse_arch2_sequential_filtering(img_bgr):
    def _run_unet_sup(img_bgr_in):
        ...
        img_rs_bgr = cv2.cvtColor(img_rs, cv2.COLOR_RGB2BGR)  # back to BGR for tf_full pipeline
        # ← use unet_model (SMP ResNet34) instead of unet_cust_model
        prob = run_unet_full(unet_model, img_rs_bgr)
        return prob  # float32 in [0,1], well-calibrated
    ...
    # probability map from the calibrated SMP model
    unet_prob = _run_unet_sup(img_bgr)
    # binary mask for overlap filtering
    unet_mask = (unet_prob > SUPERVISOR_UNET_MASK_THRESH).astype(np.uint8)
    # float prob map resized to original dims for display
    pred_mask = cv2.resize(unet_prob, (W, H), interpolation=cv2.INTER_LINEAR)

    mH, mW = unet_mask.shape
    ob, os_, oc = [], [], []
    for pb, ps, pc in zip(yb, ys, yc):
        ...
        if roi.sum() / max(roi.size, 1) >= SUPERVISOR_OVERLAP_THRESH:
            ob.append(pb)
            os_.append(ps)
            oc.append(pc)
...
<div class="pip-card" data-modal-open="modal-unet-train">
  ...
  <div class="card-body">
    ...
    <div class="stat-row"><span class="stat-label">Optimizer</span><span classx="stat-val">AdamW (lr=2e-4)</span></div>
...
<div class="section-wrap">
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;margin-bottom:14px;">
    ...
    <div class="arch-card">
      ...
      <div style="font-size:0.77rem;color:var(--text-muted);line-height:1.5;margin-bottom:0.5rem;">
        YOLO first generates all boxes. High-confidence detections (≥ 0.50) are kept unchanged.
        For each uncertain box, the ROI is cropped and fed to U-Net; a foreground ratio is computed.
        If ratio ≥ 0.30 → score boosted (+0.06 × ratio); otherwise → score penalised (−0.18 × (1 − ratio)).
      </div>
      <div class="flow">
      ...
      <div class="arch-metrics-mini">
        <div class="mini-metric"><span class="mk">P</span><span class="mv">0.854</span></div>
    ...
    <div class="arch-card">
      ...
      <div class="arch-family precision">⬆ Precision-Oriented</div>
      <div style="font-size:0.77rem;color:var(--text-muted);line-height:1.5;margin-bottom:0.5rem;">
        A custom full-image U-Net generates a binary segmentation mask. Each YOLO box is
        spatially compared with this mask; a box is kept only if its region contains at least
        15% foreground pixels. Boxes below this threshold are completely discarded — hard gate,
        no score adjustment. Achieves the highest F1 of all architectures.
      </div>
      <div class="flow">
        <div class="flow-box yolo">YOLO boxes</div><div class="flow-arr">→</div>
    ...
    <div class="arch-card">
      ...
      <div class="arch-family precision">⬆ Precision-Oriented</div>
      <div style="font-size:0.77rem;color:var(--text-muted);line-height:1.5;margin-bottom:0.5rem;">
        YOLO and a full-image U-Net run in parallel. For every detection the pixel overlap ratio
        between the box and the U-Net probability map is measured. Overlap ≥ 0.40 → score boosted
        (+0.30 × ratio); otherwise → score penalised (−0.15 × (1 − ratio)). Adjustment is uniform
        across all boxes. Outputs the pixel mask alongside detections.
      </div>
      <div class="flow">
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;">
    ...
    <div class="arch-card">
      ...
      <div class="arch-family precision">⬆ Precision-Oriented</div>
      <div style="font-size:0.77rem;color:var(--text-muted);line-height:1.5;margin-bottom:0.5rem;">
        High-confidence boxes (≥ 0.70) pass through unchanged. For uncertain detections, U-Net
        processes the cropped ROI and extracts three features: foreground ratio R<sub>fg</sub>,
        blob consistency B<sub>size</sub>, and mean probability M<sub>mean</sub>. A logistic
        regression layer trained on validation data maps (YOLO conf, R<sub>fg</sub>, B<sub>size</sub>,
        M<sub>mean</sub>) → P(TP). Boxes below CONF_OPER (0.35) are dropped. Highest precision of all.
      </div>
      <div class="flow">
    ...
    <div class="arch-card">
      ...
      <div style="font-size:0.77rem;color:var(--text-muted);line-height:1.5;margin-bottom:0.5rem;">
        For each YOLO box, two U-Net signals are computed: mean probability inside the box
        and IoU with the nearest blob. An adaptive sigmoid α gate (ALPHA_MIN=0.55) blends YOLO
        and U-Net signals: S<sub>final</sub> = α·S<sub>yolo</sub> + (1−α)·S<sub>unet</sub>.
        Additionally, new detections are injected from unmatched U-Net blobs that meet spatial
        constraints (confidence, distance, overlap). Achieves a balanced precision–recall trade-off.
      </div>
      <div class="flow">
        <div class="flow-box yolo">YOLO</div><div class="flow-arr">+</div>
      ...
      <div class="arch-metrics-mini">
        <div class="mini-metric"><span class="mk">P</span><span class="mv">0.777</span></div>
        <div class="mini-metric"><span class="mk">R</span><span class="mv">0.797</span></div>
        <div class="mini-metric"><span class="mk">mAP</span><span class="mv">0.794</span></div>
        <div class="mini-metric"><span class="mk">F1</span><span class="mv">0.787</span></div>
      </div>
    </div>
    ...
    <div class="arch-card">
      ...
      <div class="arch-family recall">⬆ Recall-Oriented</div>
      <div style="font-size:0.77rem;color:var(--text-muted);line-height:1.5;margin-bottom:0.5rem;">
        Attention fusion as in Arch. 5, but without detection injection. Instead, a rescue
        mechanism targets low-confidence YOLO detections (conf &lt; 0.60) with strong U-Net IoU
        support (&gt; 0.30): score is boosted by 0.18 × IoU before blending.
        IoU weight is raised (W_IOU=0.60) to give more influence to spatial agreement.
        Best mAP@0.5 and highest recall of all architectures.
      </div>
      <div class="flow">
        <div class="flow-box yolo">YOLO</div><div class="flow-arr">+</div>
      ...
      <div class="arch-metrics-mini">
        ...
        <div class="mini-metric"><span class="mk">R</span><span class="mv best">0.826 ★</span></div>
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>0.8023</td><td>0.8113</td><td>0.7887</td><td>0.7998</td>
        </tr>
        <tr>
          <td>Arch. 1: Score Refinement</td>
          <td style="color:#f59e0b">Precision</td>
          <td>0.7843</td><td>0.8539</td><td>0.7655</td><td>0.8073</td>
        </tr>
        <tr>
          <td>Arch. 2: Sequential Filtering</td>
          <td style="color:#f59e0b">Precision</td>
          <td>0.7475</td><td>0.8376</td><td>0.7848</td>
          <td><strong style="color:#00c8ff">0.8104 ★</strong></td>
        </tr>
        <tr>
          <td>Arch. 3: Parallel Score Refinement</td>
          <td style="color:#f59e0b">Precision</td>
          <td>0.7932</td><td>0.8315</td><td>0.7805</td><td>0.8052</td>
        </tr>
        <tr>
          <td>Arch. 4: Learned Confidence Fusion</td>
          <td style="color:#f59e0b">Precision</td>
          <td>0.6605</td>
          <td><strong style="color:#00ff9d">0.9263 ★</strong></td>
          <td>0.6858</td><td>0.7881</td>
        </tr>
        <tr>
          <td>Arch. 5: Attention Fusion + Injection</td>
          <td style="color:#34d399">Recall</td>
          <td>0.7944</td><td>0.7767</td><td>0.7970</td><td>0.7867</td>
        </tr>
        <tr style="background:rgba(0,200,255,0.04)">
          <td><strong style="color:#00c8ff">Arch. 6: Controlled Recall Recovery</strong></td>
          <td style="color:#34d399">Recall</td>
          <td><strong style="color:#00c8ff">0.7963 ★</strong></td>
          <td>0.6195</td>
          <td><strong style="color:#00ff9d">0.8262 ★</strong></td>
          <td>0.7081</td>
        </tr>
      </tbody>
    </table>
